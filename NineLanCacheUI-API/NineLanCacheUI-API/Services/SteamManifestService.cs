...
using NineLanCacheUI_API.Steam.SteamProto;
using SteamKit2;
namespace NineLanCacheUI_API.Services
{
    public class SteamManifestService
    {
        ...
        public SteamManifestService(IConfiguration configuration, IServiceProvider services, IHttpClientFactory httpClientFactory, ILogger<SteamManifestService> logger)
        {
            ...
            var deveLanCacheUIDataDirectory = configuration["LanCacheUIDataDirectory"] ?? string.Empty;
                ...
                await fallbackPolicy.WrapAsync(retryPolicy).ExecuteAsync(async () =>
                {
                    try
                    {
                        ...
                        await using (var scope = _services.CreateAsyncScope())
                        {
                            ...
                            httpClient.DefaultRequestHeaders.Referrer = DownloadEventAggregationHostedService.SkipLogLineReferrer; //Add this to ensure we don't process this line again
