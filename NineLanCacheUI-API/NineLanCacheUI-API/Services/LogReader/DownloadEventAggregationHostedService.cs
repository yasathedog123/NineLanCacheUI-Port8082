...
namespace NineLanCacheUI_API.Services.LogReader
{
    public class DownloadEventAggregationHostedService : BackgroundService
    {
        ...
        private readonly ILogger<DownloadEventAggregationHostedService> _logger;
        ...
        public DownloadEventAggregationHostedService(IServiceProvider services,
            IConfiguration config,
            SteamManifestService steamManifestService,
            ILogger<DownloadEventAggregationHostedService> logger)
        {
            _services = services;
            _config = config;
        ...
        private async Task GoRun(CancellationToken stoppingToken)
        {
            ...
            await using (var scope = _services.CreateAsyncScope())
            {
                ...
                if (Convert.ToBoolean(_config["SkipLinesBasedOnBytesRead"]) && long.TryParse(totalByteReadSetting?.Value, out var result))
                {
                    TotalBytesRead = result;
                }
            }

            var logFilePath = _config["LanCacheLogsDirectory"];
            if (logFilePath == null)
            {
